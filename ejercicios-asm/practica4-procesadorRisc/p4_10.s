...
.code
;cargo variables
daddi $t4, $zero, $zero  ;inicializo en 0 el puntero de la cadena
daddi $t5, $t5, 16 ;cantidad de caracteres de la cadena (para decrementar)
daddi $a0, $zero, cadena ;1)copio la direccion de cadena en $a0
ld $t2, car($zero) ;caracter a  buscar
ld $t1, cant($zero) ;contador de repeticiones
dadd $t4, $a0, $zero ;2)copio la direccion de inicio de la cadena (para ir sacando caracteres)

;loop que recorre la cadena
loop: lbu $t6, 0($t4) ;saco de la cadena el primer caracter (guarda en $t6, desplazamientos 0, el valor de la direccion de $t4)
daddi $t4, $t4, 1  ;incremento para obtener el siguiente elemento 
daddi $t6, $t6, -1 ;decremento la longitud del string recorrida
