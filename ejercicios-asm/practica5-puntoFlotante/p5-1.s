...
s.d f3, res1(r0)  ;guarda suma en res1
s.d f4, res2(r0)  ;guarda mul en res2
halt 
...
;1) se genera en la instrucción "add.d f3, res1($zero)" porque f2 todavía no se almacenó.
;2) se genera en "s.d f3,res1,($zero)" porque quiere que guardemos algo que la ejecucción anterior no terminó de almacenar.
;3) se genera en "s.d f4, res2($zero)" pero debido a que como la instrucción de multiplicar demanda mucho, para cuando esta se intenta ejecutar
...
;en nuestras dos operaciones aritmeticas.
;-------------------------------------------------------------------------------------------------------------------
...
;RESPUESTA: porque le da un poco más de tiempo a guardar el resultado (?)